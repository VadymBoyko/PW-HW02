...
from console_interface import ShowContactResult, ShowNoteResult, ShowHelpCommandsResult
...
def days_to_bd(promt: str):
    ...
    if res:
        return ShowContactResult.show_info(res)
    else:
        return messages.get(14)

@error_processor
def search(promt: str):
    return ShowContactResult.show_info(Fore.BLUE + book.search(promt) + Fore.RESET)
...
def find_note(promt: str):
    return ShowNoteResult.show_info(Fore.BLUE + notes.find_note(promt) + Fore.RESET)
...
def show_all_tags(promt: str):
    ...
    for r in res:
        res1 += str(r) + ' '
    return ShowNoteResult.show_info(Fore.BLUE + res1 + Fore.RESET)

@error_processor
def get_all_notes(promt: str):
    return ShowNoteResult.show_info(Fore.BLUE + notes.get_all_notes() + Fore.RESET)
...
def find_user(promt: str):
    return ShowContactResult.show_info(Fore.BLUE + str(book.find_user(promt)) + Fore.RESET)

@error_processor
def find_notes_by_tag(promt: str):
    try:
        res = notes.find_notes_tags(promt)
        return ShowNoteResult.show_info(Fore.BLUE + str(res) + Fore.RESET)
    except:
...
def show_help(promt: str):
    ...
    return ShowHelpCommandsResult.show_info(Fore.GREEN + Style.BRIGHT + res + Style.RESET_ALL + Fore.RESET)
